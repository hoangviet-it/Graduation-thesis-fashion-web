...
import LoadingMini from '../utils/loading/LoadingMini'
...
export default function CreateProduct() {
    ...
    const [refresh, setRefresh] = state.productsAPI.callback1
    ...
    // refresh lại form khi đã thêm thành công
    useEffect(() =>{
        if(param.id){
            setOnEdit(true)
    ...
    const handleSubmit = async e =>{
        ...
        try {
            ...
            if(!size1&&!size2&&!size3&&!size4&&!size5&&!size6&&!size7&&!size8&&!size9&&!size10) return alert("Bạn chưa chọn kích cỡ sản phẩm!")

            const res = await axios.post('/api/products', {product_id, ...product, content: value, images, images1, images2, images3, size1, size2, size3, size4, size5, size6, size7, size8, size9, size10}, {
                headers: {Authorization: token}
            })
            alert(res.data.msg)
            setRefresh(!refresh)
    ...
    // xử lý mã sản phẩm tự động (product_id)
    useEffect(()=>{
        var a=[]; var b=0
        const getPro_id = ()=>{
            productAll.forEach(el=>{
                a.push(el.product_id)
            })
        }
        getPro_id()
        if(a.length!==0){
            b = (Number.parseInt((a[0].slice(2,7)),10)+1)
            if(b<10){
                setProduct_id('SP0'+ b.toString())
            }
            else if(b>=10) {
                setProduct_id('SP'+ b.toString())
            }
        }
        else {
            setProduct_id('SP01')
  ...
  return (
    ...
    <div className="title-line-create-pro-ad">
        <div className="item-tt-line-cre-pro-ad">Sản phẩm mới</div>
    </div>
        ...
        <div className="container-img">
            <div className="form-img">
                <div className="upload">
                    <input type="file" name="file" id="file_up" onChange={handleUpload} onClick={e => (e.target.value = null)} />
                    {
                ...
                <div className="upload">
                    <input type="file" name="file1" id="file_up1" onChange={handleUpload1} onClick={e => (e.target.value = null)} />
                    {
                ...
                <div className="upload">
                    <input type="file" name="file2" id="file_up2" onChange={handleUpload2} onClick={e => (e.target.value = null)} />
                    {
                ...
                <div className="upload">
                    <input type="file" name="file3" id="file_up3" onChange={handleUpload3} onClick={e => (e.target.value = null)} />
                    {
            ...
            <div className="form-create">
                <div className="wrap-size">
                    <div className="container-size contain-optSi">
                        <span style={{fontWeight:'500'}}>Chọn loại kích cỡ (size): </span>
                        <span className="option-size" onClick={optionsChoseSizeLetter}>Size chữ</span>
                        <span className="option-size" onClick={optionsChoseSizeNumber}>Size số</span>
                    </div>
                ...
                <div className="form-item">
                    <div className="row-create-summernote">
                        <span>
                            ...
                            <input type="text" name="product_id" id="product_id" required value={product_id} 
                    ...
                    <div className="summernote" style={{height: 400}}>
                        <div className="quill" ref={quillRef} />
                    </div>
                    <div className="btn-form-create-pro-ad">
                        <button className="btn-back-pro" onClick={backToProManagement}>Trở về</button>
                        <button className="btn-create-pro" onClick={handleSubmit}>Thêm</button>
                    </div>
                </div>
            </div>
        </div>
